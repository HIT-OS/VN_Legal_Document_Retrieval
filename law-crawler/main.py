...
try:
    with db.atomic():
        PDChuDe.bulk_create([PDChuDe(ten=chude["Text"], stt=chude["STT"], chude_id=chude["Value"]) for chude in chudes])
...
try:
    with db.atomic():
        PDDeMuc.bulk_create(
            [PDDeMuc(ten=demuc["Text"], stt=demuc["STT"], demuc_id=demuc["Value"], chude_id=demuc["ChuDe"]) for demuc in
             demucs])
except:
    pass
    ...
    with open("./phap-dien/demuc/" + file_name, "r") as demuc_file:
        ...
        for chuong in demuc_chuong:
            ...
            chuong_data = PDChuong(ten=chuong["TEN"],
                                   chuong_id=mapc, chimuc=chuong["ChiMuc"],
                                   stt = stt,
        ...
        for dieu in demuc_dieus:
            if len(chuongs_data) == 1:
                ...
                print(chuongs_data[0].chuong_id)
            else:
                for chuong in chuongs_data:
                    if dieu["MAPC"].startswith(chuong.chuong_id):
                        dieu["ChuongID"] = chuong.chuong_id
                        # print(chuong.chuong_id)
                        break
            mapc = dieu["MAPC"]
